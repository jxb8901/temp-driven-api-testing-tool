...
| configured report/log output | end-user `att.sh` output, if configured | Yes, by `att.sh clean` |
...
`att.sh clean` removes only artifacts generated by end users invoking `att.sh`: configured output/report/log roots, `build/docs`, and `build/att-*.tar.gz`. It MUST NOT remove `target`, `target/release`, or `dist`.
