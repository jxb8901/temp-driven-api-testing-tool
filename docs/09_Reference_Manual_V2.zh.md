...
| 模板 | 一个可复用的有序动作列表 |
| 工具 | 一个全局配置的外部可执行文件，具有命名输入和一个声明的输出格式 |
...
[`schemas/catalog.yaml`](../schemas/catalog.yaml) 使用 `att-schema-catalog/v2.5`，列出当前与兼容 schema。V2.5 当前 schema 为 `att-config/v2.5`、`att-template/v2.5` 与 `att-dbhelper/v2.5`；Tool group 继续使用 `att-tool-group/v2.2`。旧 `att-config/v2.1|v2.2` 与 `att-template/v2.3` 保持可读，但不能使用 V2.5-only 字段。
...
| 路径 | 必填/默认值 | 约束 |
|---|---|---|
| `schemaVersion` | 必填 | 当前为 `att-config/v2.5`；旧 V2.1/V2.2 文件仍可读取，但不能配置 dbhelper |
...
| tool | 需要 `call`；可选 object `saveAs`、`assert`、`timeoutMs`、`retry`；不允许 DB/render/assert-action/log-only 字段 |
...
每个工具要求 `name`、`description` 和 `command`。`command` 可以是非空标量或非空字符串列表。`output` 默认 `txt`，支持 `txt`、`yaml`、`json`、`xml`。每个参数要求 `name`、`description` 和 YAML 布尔 `required`。`argName` 可为空或一个无空白 argv token。`argNameMode` 接受 `once` 或 `repeat`，默认为 `once`。任何数量的声明参数都可定义非空 `delimit`，每个 delimited 占位符都必须占据一个完整命令 token。
...
`run.yaml` 使用 `schemaVersion: att-run/v2.1`，记录 ATT/构建身份、Java/OS/locale/timezone、校验模式、环境、时间戳、状态/摘要、输出路径，以及有效配置、工具组文件、工作簿、侧车、解析模板/负载、包内工具文件和 schema/catalog 版本的 SHA-256 hash。
