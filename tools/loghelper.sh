...
#    9. 結果寫入 <output_prefix>.<TID>.log，每個 TID 一個獨立文件
...
for arg in "$@"; do
    ...
    candidate=$(canonical_existing_file "$arg" 2>/dev/null || true)
    if [ -z "$candidate" ]; then
        candidate=$(canonical_existing_file "$LOG_DIR/$arg" 2>/dev/null || true)
    fi
    if [ -z "$candidate" ]; then
...
OUTPUT_PREFIX=$(canonical_output_path "$OUTPUT_PREFIX") || fail "the output directory does not exist."
...
while IFS=$'\t' read -r tid has_start has_end; do
    ...
    output_file=$(canonical_output_path "$output_file") || fail "could not resolve output path for TID '$tid'."
