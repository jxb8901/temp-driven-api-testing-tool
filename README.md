# ATT 3.1.0 - Automated Testing Tool

ATT V3.1.0 treats a Flow as a statically resolved, reusable group of Template Actions. Flow Actions use the same `CASE`, `ACTIONS`, `RUN`, `TOOL`, `DB`, and current `output` Context as inline Template Actions, so extracting or restoring an Action does not require rewriting its expressions.
...
- Authoring schemas remain named `att-template/v3.0` and `att-flow/v3.0`, but V3.1 deliberately replaces the earlier isolated Flow contract. V2.6, V2.5 and V2.3 Templates remain readable without semantic changes.
...
- V3 Action `runWhen` skips one statically known Action. `runAlways`, warning impact, Flow timeout/retry, dynamic dispatch, loops and parallel branches are not V3.1.0 features.
...
- V2.5 `saveAs` is an object with `path`, optional target-specific `format`, and optional `overwrite`. DB Actions require `text|json|yaml|xml`; DB text is a deterministic SQL*Plus-style table/update summary. Process Tools additionally support exact `raw` stdout, while built-ins default to `text`.
...
- Every expression-bearing surface uses one engine. Context references inside `#{...}` calls must use `${...}`, for example `#{length(${CASE.VARS.reference})}`; bare `CASE.*`, `ACTIONS.*`, `input.*`, and other scope paths are rejected instead of being inferred. Quoted paths remain literal strings. Case runtime fields can invoke built-ins or configured Tools; the dedicated report-filename and Tool-command scopes expose only their documented values and built-ins.
...
See the [ATT V3.1.0 Reference Manual](docs/09_Reference_Manual_V3.md) and [ATT V3.1.0 Quick Start](docs/08_Quick_Start_V3.md) for operation and authoring guidance.
