...
- Template and Flow Actions use canonical `EXEC`/`META` roots plus the current Action's local `output`. Root-level `TOOL.*` and `DB.*` remain compatibility/transient views only; `EXEC.TOOL`, `EXEC.DB`, `EXEC.MQ`, `EXEC.OUTPUT`, and `EXEC.STAGES` are not 3.4.2 Context nodes. The complete property reference is in the V3 Reference Manual.
