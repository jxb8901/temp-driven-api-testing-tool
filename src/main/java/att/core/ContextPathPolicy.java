...
public final class ContextPathPolicy {
    ...
    private static final Set<String> EXPRESSION_ROOTS = roots("EXEC", "META", "CASE", "RUN", "ACTIONS", "TOOL", "DB", "output");
    private ContextPathPolicy() { }

    public static boolean isExplicitRoot(String root) { return root != null && EXPRESSION_ROOTS.contains(root); }
    public static boolean isCanonicalRoot(String root) { return root != null && CANONICAL_ROOTS.contains(root); }
    ...
    public static Scope classify(String path) {
        ...
        if (path.startsWith("CASE.") || path.startsWith("CASE[") || path.startsWith("RUN.")
                || path.startsWith("RUN[") || path.startsWith("ACTIONS.") || path.startsWith("ACTIONS[")) return Scope.LEGACY_ALIAS;
        return Scope.OTHER;
    }
    ...
    public static String firstSegment(String path) {
        int dot = path.indexOf('.'); int bracket = path.indexOf('['); int end = dot < 0 ? path.length() : dot;
        if (bracket >= 0 && bracket < end) end = bracket; return path.substring(0, end);
    }
